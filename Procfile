...
web: python manage.py esperar_db && python manage.py migrate --noinput && python manage.py cargar_ruta_enfocada && python manage.py cargar_biblioteca && python manage.py podar_catalogo --si && gunicorn config.wsgi --bind 0.0.0.0:$PORT --workers 2 --log-file -
